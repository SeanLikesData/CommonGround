...
const STAGES: Stage[] = [
  { key: "rf", label: "RF", cx: 50, cy: 32, w: SENSOR_W, h: SENSOR_H, color: "#22d3ee" },
  { key: "ugs", label: "UGS", cx: CENTER, cy: 32, w: SENSOR_W, h: SENSOR_H, color: "#22d3ee" },
  { key: "drone", label: "Drone", cx: 230, cy: 32, w: SENSOR_W, h: SENSOR_H, color: "#22d3ee" },
  ...
  { key: "agent", label: "Agent loop", cx: CENTER, cy: 580, w: NODE_W, h: NODE_H, color: "#f472b6" },
  { key: "memories", label: "memories.md", cx: CENTER, cy: 670, w: NODE_W, h: NODE_H, color: "#67e8f9" },
];
...
function buildEdgePath(from: Stage, to: Stage): string {
...
const EDGES: Edge[] = (
  [
    ["e-rf", "rf", "signals", "#22d3ee", 2],
    ["e-ugs", "ugs", "signals", "#22d3ee", 2],
    ["e-drone", "drone", "signals", "#22d3ee", 2],
    ["e-1", "signals", "reportgen", "#a78bfa", 3],
    ["e-2", "reportgen", "reports", "#34d399", 3],
    ["e-3", "reports", "kgupdate", "#fb923c", 2],
    ["e-4", "kgupdate", "neo4j", "#facc15", 2],
    ["e-5", "neo4j", "agent", "#f472b6", 2],
    ["e-6", "agent", "memories", "#67e8f9", 2],
  ] as const
).map(([id, from, to, color, tokens]) => ({
  id,
  from,
  to,
  color,
  tokens,
  d: buildEdgePath(STAGE_BY_KEY[from], STAGE_BY_KEY[to]),
}));

function sensorStageFor(source: SpotSource): StageKey | null {
  if (source === "rf") return "rf";
  if (source === "ugs" || source === "i-ugs" || source === "human") return "ugs";
...
const DOWNSTREAM_OF_AGENT: Array<{ key: StageKey; delay: number }> = [
  { key: "agent", delay: 0 },
  { key: "memories", delay: 350 },
];
...
export default function FlowDiagram() {
  ...
  useEffect(() => {
    if (memory.length > prevMemoryCountRef.current) {
      ...
      for (let i = 0; i < delta; i++) {
        for (const stage of DOWNSTREAM_OF_AGENT) {
          bump(stage.key, stage.delay);
        }
      }
    }
  ...
  // Cycle the agent loop on its own ambient cadence so it visibly "ticks"
  // even when no new memories arrive.
  useEffect(() => {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2 p-3">
      ...
      <div className="grid grid-cols-3 gap-2 border-t border-zinc-800 pt-2 text-[10px] text-zinc-500">
        <div>
        ...
        <div>
          <div className="text-zinc-300">{memory.length}</div>
          <div>memories</div>
        </div>
      </div>
    </div>
